...
export default function ListStandard() {
    ...
    let [nameDress, setNameDress] = useState('');
    let [nameTypeDress, setNameTypeDress] = useState('STANDARD');
    ...
    const nextPage = async () => {
        page += 1;
        if (page < standards.totalPages) {
            await setPageFunction(page).then(setStandards(await getListDress(page, nameDress, nameTypeDress, nameStatus, headers)));
        } else {
            setPage(page - 1);
        }
    }
                                            ...
                                            {standards.content.map((st, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            <p className="text-xs text-center font-weight-bold mb-0">ST-{st.idDress}</p>
                                                        </td>
                                                        <td>
